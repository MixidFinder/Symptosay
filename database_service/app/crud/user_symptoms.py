...
async def record_user_symptom(db: AsyncSession, record: UserSymptomCreate) -> UserSymptom:
...
async def get_user_symptoms(db: AsyncSession, user_id: int, skip: int = 0, limit: int = 100) -> list[UserSymptom]:
    stmt = select(UserSymptom).where(UserSymptom.user_id == user_id).offset(skip).limit(limit)
    result = await db.execute(stmt)
    return result.scalars().all()


async def update_user_symptom(db: AsyncSession, record_id: int, update: UserSymptomUpdate) -> UserSymptom | None:
...
async def delete_user_symptom(db: AsyncSession, record_id: int) -> UserSymptom | None:
    stmt = select(UserSymptom).where(UserSymptom.id == record_id)
    result = await db.execute(stmt)
    us = result.scalar_one_or_none()
    if not us:
