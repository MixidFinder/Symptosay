...
async def record_user_symptom(
    record: UserSymptomCreate, db: Annotated[AsyncSession, Depends(get_user_symptoms_db)]
) -> UserSymptomOut:
...
async def read_user_symptoms(
    user_id: int, skip: int = 0, limit: int = 100, db: Annotated[AsyncSession, Depends(get_user_symptoms_db)] = None
) -> list[UserSymptomOut]:
...
async def update_user_symptom(
    record_id: int, update: UserSymptomUpdate, db: Annotated[AsyncSession, Depends(get_user_symptoms_db)]
) -> UserSymptomOut:
...
async def delete_user_symptom(
    record_id: int, db: Annotated[AsyncSession, Depends(get_user_symptoms_db)]
) -> UserSymptomOut:
