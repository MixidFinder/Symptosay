...
async def read_symptoms(
    skip: int = 0, limit: int = 100, db: Annotated[AsyncSession, Depends(get_symptoms_db)] = None
) -> list[SymptomOut]:
...
async def create_symptoms_batch(
    batch: SymptomsBatchCreate, db: Annotated[AsyncSession, Depends(get_symptoms_db)]
) -> list[SymptomOut]:
